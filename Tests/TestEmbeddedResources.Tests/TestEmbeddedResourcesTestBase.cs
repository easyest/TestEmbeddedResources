...
namespace TestEmbeddedResources.Tests
{
    public abstract class TestEmbeddedResourcesTestBase : AbpIntegratedTestBase<TestEmbeddedResourcesTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TestEmbeddedResourcesTestBase()
        {
        ...
        protected void UsingDbContext(Action<TestEmbeddedResourcesDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestEmbeddedResourcesDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestEmbeddedResourcesDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestEmbeddedResourcesDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestEmbeddedResourcesDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEmbeddedResourcesDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestEmbeddedResourcesDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEmbeddedResourcesDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestEmbeddedResourcesDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestEmbeddedResourcesDbContext, Task<T>> func)
        {
            T result;
